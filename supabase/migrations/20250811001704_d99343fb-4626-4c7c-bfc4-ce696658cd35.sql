-- Fixed RLS and Multi-tenancy Setup
-- Add org_id to existing tables and create safety tables
...
-- Create RLS policies for safety tables with correct org_id reference
CREATE POLICY "safety_signals_isolated" ON public.safety_signals
    FOR ALL USING (safety_signals.org_id = current_org());

CREATE POLICY "safety_incidents_isolated" ON public.safety_incidents  
    FOR ALL USING (safety_incidents.org_id = current_org());
