...
create trigger update_antitheft_zones_updated_at
...
-- signals policies
create policy if not exists signals_select_auth on public.antitheft_signals
for select using (auth.role() = 'authenticated');

create policy if not exists signals_insert_service on public.antitheft_signals
for insert with check (((current_setting('request.jwt.claims', true))::jsonb ->> 'role') = 'service_role');

-- incidents policies
create policy if not exists incidents_select_auth on public.antitheft_incidents
for select using (auth.role() = 'authenticated');

create policy if not exists incidents_insert_service on public.antitheft_incidents
for insert with check (((current_setting('request.jwt.claims', true))::jsonb ->> 'role') = 'service_role');

-- zones policies (admin/operator manage, service can write)
create policy if not exists zones_select_auth on public.antitheft_zones
for select using (auth.role() = 'authenticated');

create policy if not exists zones_insert_admin_operator_service on public.antitheft_zones
for insert
with check (
...
create policy if not exists zones_update_admin_operator_service on public.antitheft_zones
for update
using (
  has_role(auth.uid(), 'admin'::app_role)
  or has_role(auth.uid(), 'operator'::app_role)
...
create policy if not exists zones_delete_admin on public.antitheft_zones
for delete using (has_role(auth.uid(), 'admin'::app_role));

-- receipts policies (admin/operator read, service/admin/operator write)
create policy if not exists receipts_select_admin_operator on public.antitheft_receipts
for select using (has_role(auth.uid(), 'admin'::app_role) or has_role(auth.uid(), 'operator'::app_role));

create policy if not exists receipts_insert_admin_operator_service on public.antitheft_receipts
