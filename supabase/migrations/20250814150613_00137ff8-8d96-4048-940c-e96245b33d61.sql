...
CREATE POLICY "personal_data_inventory_isolated" ON public.personal_data_inventory
  FOR ALL USING (org_id = current_org());

CREATE POLICY "anonymization_jobs_isolated" ON public.anonymization_jobs
  FOR ALL USING (org_id = current_org());
...
CREATE POLICY "encryption_keys_isolated" ON public.encryption_keys
  FOR ALL USING (org_id = current_org());

CREATE POLICY "data_access_logs_isolated" ON public.data_access_logs
  FOR ALL USING (org_id = current_org());

CREATE POLICY "consent_records_isolated" ON public.consent_records
  FOR ALL USING (org_id = current_org());
...
-- Insert sample personal data inventory
...
AS $$
  INSERT INTO public.data_access_logs (
    ...
    data_subject_id, purpose, legal_basis, org_id
  ) VALUES (
    auth.uid(), _access_type, _resource_type, _resource_id,
    _data_subject_id, _purpose, _legal_basis, current_org()
  )
  RETURNING id;
$$;