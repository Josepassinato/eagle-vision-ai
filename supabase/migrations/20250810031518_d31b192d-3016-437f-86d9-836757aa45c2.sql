-- Re-run with corrected policy creation
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY event_clips_read_authenticated ON storage.objects FOR SELECT TO authenticated USING (bucket_id = ''event_clips'')';
  END IF;
END $$;
