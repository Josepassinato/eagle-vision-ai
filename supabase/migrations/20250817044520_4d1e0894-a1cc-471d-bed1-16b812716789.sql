...
CREATE TABLE public.privacy_settings (
  ...
  privacy_mode TEXT DEFAULT 'no_bio' CHECK (privacy_mode IN ('no_bio', 'opt_in')),
  face_blur_enabled BOOLEAN DEFAULT true,
  plates_blur_enabled BOOLEAN DEFAULT true,
  anonymous_analytics BOOLEAN DEFAULT true,
  embeddings_on_device BOOLEAN DEFAULT true,
  data_retention_days INTEGER DEFAULT 30,
  auto_deletion_enabled BOOLEAN DEFAULT true,
  consent_required BOOLEAN DEFAULT true,
  signage_config JSONB DEFAULT '{}',
...
-- RLS Policies
CREATE POLICY "privacy_settings_tenant_isolation" ON public.privacy_settings FOR ALL USING (tenant_id = current_org());
...
CREATE POLICY "service_role_privacy_settings" ON public.privacy_settings FOR ALL USING (((current_setting('request.jwt.claims'::text, true))::jsonb ->> 'role'::text) = 'service_role'::text);
...
-- Function to calculate LGPD retention date
CREATE OR REPLACE FUNCTION public.calculate_lgpd_retention()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    ...
    SELECT ps.data_retention_days INTO retention_days
    FROM public.privacy_settings ps
    WHERE ps.tenant_id = NEW.tenant_id;
    
    NEW.retention_until = NEW.consent_date + (retention_days || ' days')::INTERVAL;
    RETURN NEW;
END;
$$;

-- Trigger for LGPD retention calculation
CREATE TRIGGER calculate_lgpd_retention_trigger
    BEFORE INSERT ON public.lgpd_compliance
    FOR EACH ROW
    EXECUTE FUNCTION public.calculate_lgpd_retention();
